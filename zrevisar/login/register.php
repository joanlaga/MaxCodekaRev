...
if ($_REQUEST['Submit'] == 'Registrar')
{
   if (strlen($_REQUEST['email']) < 5)
   {
    die ("Incorrect email. Please enter valid email address..");
    }
   if (strcmp($_REQUEST['pass1'],$_REQUEST['pass2']) || empty($_REQUEST['pass1']) )
	{ 
	//die ("Password does not match");
	die("ERROR: Password does not match or empty..");
	}
	
	$rs_duplicates = mysql_query("select id from users where user_email='$_REQUEST[email]'");
	...
	mysql_query("INSERT INTO users
				  ...
				  VALUES
				  ('$_REQUEST[email]','$md5pass','$_REQUEST[country]',now(),'$activ_code','$_REQUEST[full_name]','1')") or die(mysql_error());
...
";
	...
	mail($_REQUEST['email'], "Registro de usuario", $message, "From: Sistema de Facturación <administrador@$host>\r\n" );
	unset($_SESSION['ckey']);
	echo("Gracias por registrase!, un email fue enviado a su correo con los datos de activaci�n...");	
	exit;
}	
