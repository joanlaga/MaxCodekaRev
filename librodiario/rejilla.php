...
<html>
	...
	<body onload=inicio()>	
			...
			<input type="hidden" name="numfilas" id="numfilas" value="<?php echo $filas?>">
				...
				//		 while ($contador < mysql_fetch_array($res_resultado)) {
								if ($contador % 2) { $fondolinea="itemParTabla"; } else { $fondolinea="itemImparTabla"; }?>
									...
									<?php if (mysql_result($res_resultado,$contador,"tipodocumento")==1) { 
											...
											//$nombrecomercial=mysql_result($rs_proveedor,0,"nombre"); //falla Warning: mysql_result(): Unable to jump to row 0 on MySQL result index jonalaga
											$nombrecomercialA=mysql_fetch_array($rs_proveedor);
											$nombrecomercial=$nombrecomercialA['nombre'];
											$movimiento="Compra"; 
								} else { 
